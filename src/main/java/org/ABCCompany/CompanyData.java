...
public class CompanyData {
    ...
    public static void main(String[] args) {
        ...
        companyData.findManagerWithLessSalary(empMap);
        companyData.findHierarchy(empMap);
    ...
    public void findManagerWithLessSalary(Map<Integer, Employee> empMap) {
        ...
        for (Map.Entry<Integer, List<Employee>> mgMap : mngrEmpMap.entrySet()) {
            Double avgSal = mgMap.getValue().stream().map(e -> e.getSalary()).mapToDouble(Integer::doubleValue).average().getAsDouble();
            Integer mngId = mgMap.getKey();
            Double mngSal = Double.valueOf(empMap.get(mngId).getSalary());
            Double per = (((mngSal - avgSal) / avgSal) * 100);
            final DecimalFormat df = new DecimalFormat("0.00");
            df.setRoundingMode(RoundingMode.UP);

            Double diff = Double.valueOf(df.format(per));
            if (diff <= 20) {
            ...
            System.out.println("per ----> " + per + " Employee Id: " + mngId + " average sal :" + avgSal + "  mngSal:  " + mngSal);
        ...
        mngWithLessSal.entrySet().stream().forEach(hm -> {
            System.out.println("Employee Id: " + hm.getValue().getId() + " and Name: " + hm.getValue().getFirstName() + " " + hm.getValue().getLastName() + " also the difference in salary is by " + hm.getKey() + "%");
        });
        System.out.println(" *********** ");
        ...
        mngWithMoreSal.entrySet().stream().forEach(hm -> {
            System.out.println("Employee Id: " + hm.getValue().getId() + " and Name: " + hm.getValue().getFirstName() + " " + hm.getValue().getLastName() + " also the difference in salary is by " + hm.getKey() + "%");
        });
